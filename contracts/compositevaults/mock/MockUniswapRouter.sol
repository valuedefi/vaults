...
contract MockUniswapRouter is IUniswapRouter {
    ...
    IERC20 univ2LpToken;
    mapping(address => mapping(address => uint)) swapRate;
    ...
    function setRate(address _input, address _output, uint rate) public {
        swapRate[_input][_output] = rate;
    }

    function getRate(address _input, address _output) public view returns (uint _rate) {
        _rate = swapRate[_input][_output];
    ...
    function _swap(
        ...
        uint256 rate = getRate(path[0], path[path.length - 1]);
        uint256 amountOut = amountIn.mul(rate).div(1e18);
